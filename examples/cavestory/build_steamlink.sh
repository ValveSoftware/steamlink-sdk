#!/bin/bash
#

TOP=$(cd `dirname "${BASH_SOURCE[0]}"` && pwd)
if [ "${MARVELL_SDK_PATH}" = "" ]; then
	MARVELL_SDK_PATH="$(cd "${TOP}/../.." && pwd)"
fi

# Get the code
git clone https://github.com/nxengine/nxengine-evo.git
cd nxengine-evo

# Make the extract binary
mkdir build
cd build
cmake ..
make extract
cd ..
rm -rf build

# Download and extract game files
wget -nc https://github.com/nxengine/translations/releases/download/v1.8/all.zip 
unzip -n all.zip
wget -nc http://www.cavestory.org/downloads/cavestoryen.zip
unzip -n cavestoryen.zip
cd CaveStory
../bin/extract
cp -r data/* ../data/
cd ..


if [ "${MARVELL_ROOTFS}" = "" ]; then
	source "${MARVELL_SDK_PATH}/setenv.sh" || exit 1
fi
cd "${TOP}/nxengine-evo"


# Build the game
mkdir build && cd build
MARVELL_SDK_PATH="${MARVELL_SDK_PATH}" cmake -DMARVELL_SDK_PATH="${MARVELL_SDK_PATH}" -DCMAKE_TOOLCHAIN_FILE="${TOP}/../../toolchain/steamlink-toolchain.cmake" -DCMAKE_BUILD_TYPE=Release ..
make || exit 2

export DESTDIR="${TOP}/steamlink/apps/CaveStory"

# Copy the files to the app directory
mkdir -p "${DESTDIR}"
cp -v -r "${TOP}/nxengine-evo/data" "${DESTDIR}/"
cp -v "${TOP}/nxengine-evo/bin/nx" "${DESTDIR}"

# Create the table of contents and icon
cat >"${DESTDIR}/toc.txt" <<__EOF__
name=Cave Story
icon=icon.png
run=nx
__EOF__

base64 -d >"${DESTDIR}/icon.png" <<__EOF__

__EOF__

# Pack it up
name=$(basename ${DESTDIR})
pushd "$(dirname ${DESTDIR})"
tar zcvf $name.tgz $name || exit 3
rm -rf $name
popd

# All done!
echo "Build complete!"
echo
echo "Put the steamlink folder onto a USB drive, insert it into your Steam Link, and cycle the power to install."
