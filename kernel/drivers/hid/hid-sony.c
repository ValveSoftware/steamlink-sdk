 ...
 *  Copyright (c) 2014 Frank Praznik <frank.praznik@oh.rr.com>
 */
...
static int sixaxis_set_operational_bt(struct hid_device *hdev)
{
	...
	ret = hdev->hid_output_raw_report(hdev, buf, sizeof(report),
					HID_FEATURE_REPORT);
...
static int dualshock4_set_operational_bt(struct hid_device *hdev)
{
	...
	ret = hdev->hid_get_raw_report(hdev, 0x02, buf, DS4_REPORT_0x02_SIZE,
				HID_FEATURE_REPORT);
...
static void buzz_set_leds(struct hid_device *hdev, const __u8 *leds)
{
	...
	value[5] = 0x00;
	value[6] = 0x00;
	usbhid_submit_report(hdev, report, HID_REQ_SET_REPORT);
}
...
static int sony_leds_init(struct sony_sc *sc)
{
	 ...
	 * flashing again.  They will be set when the PS button is pushed.
	 */
...
static int sony_check_add(struct sony_sc *sc)
{
	...
	if ((sc->quirks & DUALSHOCK4_CONTROLLER_BT) ||
		 ...
		 * offset 4.
		 */
		ret = sc->hdev->hid_get_raw_report(sc->hdev, 0xf2, buf,
...
static const struct hid_device_id sony_devices[] = {
	...
	/* Sony Dualshock 4 Slim controllers for PS4 */
	{ HID_USB_DEVICE(USB_VENDOR_ID_SONY, USB_DEVICE_ID_SONY_PS4_SLIM_CONTROLLER),
		.driver_data = DUALSHOCK4_CONTROLLER_USB },
	{ HID_BLUETOOTH_DEVICE(USB_VENDOR_ID_SONY, USB_DEVICE_ID_SONY_PS4_SLIM_CONTROLLER),
		.driver_data = DUALSHOCK4_CONTROLLER_BT },
	{ }
};
