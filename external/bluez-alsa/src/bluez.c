...
static void bluez_mediaplayer_method_call(GDBusConnection *conn, const gchar *sender,
		const gchar *path, const gchar *interface, const gchar *method, GVariant *params,
		GDBusMethodInvocation *invocation, void *userdata) {
	(void)conn;
	(void)sender;
	...
	warn("Unsupported mediaplayer method: %s", method);
}

static GVariant *bluez_mediaplayer_get_property(GDBusConnection *conn, const gchar *sender,
...
static gboolean bluez_mediaplayer_set_property(GDBusConnection *conn, const gchar *sender,
		const gchar *object_path, const gchar *interface_name, const gchar *property_name,
...
static void mediaplayer_free(gpointer data) {
	(void)data;
}

static const GDBusInterfaceVTable mediaplayer_vtable = {
	.method_call = bluez_mediaplayer_method_call,
	.get_property = bluez_mediaplayer_get_property,
	.set_property = bluez_mediaplayer_set_property,
};

/**
 * In order for config.a2dp_volume setting to work, we need a media player
 * registered.  This function registers a media player stub. that is functional
 * enough to get the volume setting working.
 *
 * @return On success this function returns 0. Otherwise -1 is returned. */
int bluez_register_mediaplayer(void) {
	GDBusConnection *conn = config.dbus;
	char *path;
	size_t i;
	...
	hash = g_str_hash("/MediaPlayer/BlueAlsaPlayerStub");
	if (g_hash_table_contains(config.dbus_objects, GINT_TO_POINTER(hash))) {
		debug("MediaPlayer already registered");
		goto done;
	}

	debug("Registering mediaplayer\n");

	if ((id = g_dbus_connection_register_object(conn, "/MediaPlayer/BlueAlsaPlayerStub",
												(GDBusInterfaceInfo *)&bluez_iface_mediaplayer, &mediaplayer_vtable,
												NULL, mediaplayer_free, &err)) == 0)
		goto done;

	msg = g_dbus_message_new_method_call("org.bluez", path,
										 "org.bluez.Media1", "RegisterPlayer");
	GVariantBuilder *properties = g_variant_builder_new(G_VARIANT_TYPE("a{sv}"));
	g_variant_builder_add(properties, "{sv}", "CanQuit", g_variant_new("b", FALSE));
	g_variant_builder_add(properties, "{sv}", "CanRaise", g_variant_new("b", FALSE));
	g_variant_builder_add(properties, "{sv}", "HasTrackList", g_variant_new("b", FALSE));
	g_variant_builder_add(properties, "{sv}", "Identity", g_variant_new_string("BlueAlsaPlayerStub"));

	g_dbus_message_set_body(msg, g_variant_new("(oa{sv})", "/MediaPlayer/BlueAlsaPlayerStub", properties));
	...
	if (rep != NULL)
		g_object_unref(rep);
	if (err != NULL) {
		warn("Couldn't register player: %s", err->message);
		g_dbus_connection_unregister_object(conn, id);
		g_error_free(err);
	}
