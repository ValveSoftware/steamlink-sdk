...
struct ba_transport *transport_new_a2dp(
		...
		enum bluetooth_profile profile,
		uint8_t codec,
		const uint8_t *transport_config,
		size_t transport_config_size) {
	...
	/* Assume volume setting via dbus is supported initially.
	 * Reevaluate later after an attempt to set volume is made
	 */
	t->a2dp.supports_dbus_volume = config.a2dp_volume;

	if (transport_config_size > 0) {
		t->a2dp.cconfig = malloc(transport_config_size);
		t->a2dp.cconfig_size = transport_config_size;
...
int transport_set_volume(struct ba_transport *t, uint8_t ch1_muted, uint8_t ch2_muted,
	...
	case TRANSPORT_TYPE_A2DP:
		...
		if (config.a2dp_volume && t->a2dp.supports_dbus_volume) {
			...
			success = g_dbus_set_property(config.dbus, t->dbus_owner, t->dbus_path,
					"org.bluez.MediaTransport1", "Volume", g_variant_new_uint16(volume));
			if (!success) {
				warn("a2dp_volume is not working, falling back to scaling pcm samples");
			}
			t->a2dp.supports_dbus_volume = success;
		}
