							...
							bool enable_on_device)
{
	...
	if (!(report->decl->properties & BT_GATT_CHRC_PROP_NOTIFY)) {
		error("Characteristic at 0x%04x does not support notify",
		      report->decl->handle);
		return;
	}
								...
								void *user_data)
{
	...
	for ( ; descs; descs = descs->next) {
		...
		case GATT_REPORT_REFERENCE:
			...
			DBG("Reading report reference at 0x%x", desc->handle);
...
static void external_service_char_cb(uint8_t status, GSList *chars,
								void *user_data)
{
	struct ext_report_ref_data *report_ref = user_data;
	struct hog_device *hogdev = report_ref->hogdev;
	uint16_t uuid16 = report_ref->uuid16;
	struct gatt_primary *prim = hogdev->hog_primary;
	struct report *report;
	GSList *l;
	bt_uuid_t ext_char_uuid;
	...
	bt_uuid16_create(&ext_char_uuid, uuid16);
	...
	for (l = chars; l; l = g_slist_next(l)) {
		...
		if (bt_uuid_cmp(&uuid, &ext_char_uuid) == 0) {
					...
					guint16 plen, gpointer user_data)
{
	...
	uint16_t uuid16;
	struct ext_report_ref_data *report_ref;
	...
	DBG("External report reference read, external report characteristic "
						"UUID: 0x%04x", uuid16);
	report_ref = g_new0(struct ext_report_ref_data, 1);
	report_ref->hogdev = hogdev;
	report_ref->uuid16 = uuid16;
	/* We need to search for all characteristics because if we only search for
	 * the wanted UUID it can lead to us searching descriptors from too wide
	 * a handle range and trying to enable notifications on
	 * characteristics that are not needed and that may not support it */
	gatt_discover_char(hogdev->attrib, 0x0001, 0xffff, NULL /* &uuid */,
					external_service_char_cb, report_ref /*hogdev*/);
}
